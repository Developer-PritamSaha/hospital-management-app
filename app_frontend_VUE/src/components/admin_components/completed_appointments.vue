...
async function loadPatients() {
    ...
    try {
        ...
        triggerAlert("Past Appointments Loaded Succesfully", "success", "bi-check-circle")
    } catch (err) {
        data.value.error = err.response?.data?.message || err.message
        triggerAlert("Past Appointments loading failed.", "danger", "bi-exclamation-triangle")
    } finally {
...
async function loadFilteredPatients() {
    ...
    try {
        ...
        if (err.response?.status === 404){
            triggerAlert("Searched patient(s) not found.", "info", "bi-info-circle")
        }
        else{
            triggerAlert("Patients data loading failed.", "danger", "bi-exclamation-triangle")
        }
...
async function deletePatient() {
    ...
    try {
        ...
        loadPatients()
        triggerAlert(`${patientToBeDeleted.value.full_name}(${patientToBeDeleted.value.patient_public_id}) has been deleted successfully.`,'success',"bi-check-circle")
    } catch (err) {
        triggerAlert(`${patientToBeDeleted.value.full_name}(${patientToBeDeleted.value.patient_public_id}) deletion failed.`, "danger", "bi-exclamation-triangle")
    } finally{
...
async function blockPatient(patient) {
    try {
        ...
        if(patient.is_active){
            triggerAlert(`${patient.full_name}(${patient.patient_public_id}) has been blocked successfully.`,'success',"bi-check-circle")
        } else{
            triggerAlert(`${patient.full_name}(${patient.patient_public_id}) has been unblocked successfully.`,'success',"bi-check-circle")
        }
        ...
        if(patient.is_active){
            triggerAlert(`${patient.full_name}(${patient.patient_public_id}) blocking failed.`, "danger", "bi-exclamation-triangle")
        } else{
            triggerAlert(`${patient.full_name}(${patient.patient_public_id}) unblocking failed.`, "danger", "bi-exclamation-triangle")
        }
    }
}
...
onMounted(() => {
    ...
    if(globalTemp.get('PatientEditStatus') === 'success'){
        triggerAlert(`${globalTemp.get('PatientName')} edited successfully!`, "info", "bi-check-circle")
    ...
    <div class="row g-2">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <div class="d-flex align-items-center">
                <h2 class="h4 fw-bold" style="color: #220349;">Past Appointments</h2> 
