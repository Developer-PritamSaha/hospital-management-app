...
async function loadPatientDetails() {
    try {
        const response = await axios_instance.get("/api/dashboard/admin/patient",{
            params: {
                "patient_id": globalTemp.get('patient_id')
            }
          })
        currentPatientData.value = response.data

        data.value.patient_id = response.data.patient_id
        ...
        appendAlert("Patient data loading failed.", "danger", "bi-exclamation-triangle")
    }
}
...
async function saveChanges(){
    ...
    resetInvalidFlags()
    try {
        const response = await axios_instance.patch("/api/dashboard/admin/patient", data.value)
        globalTemp.reset('patient_id')
        globalTemp.set('PatientEditStatus', "success")
        globalTemp.set('PatientName', `${data.value?.full_name.trim()}(${currentPatientData.value?.patient_public_id})` || 'Patient')
        router.replace("/dashboard/admin/patients")
    } catch(error) {
        let msg = error.response?.data?.message || "Patient Edit failed."
        if(typeof msg === 'object'){
            if(msg?.patient_id){
                msg = "Patient_id " + msg.patient_id
            }
            if(msg?.full_name){
                msg = msg.full_name
