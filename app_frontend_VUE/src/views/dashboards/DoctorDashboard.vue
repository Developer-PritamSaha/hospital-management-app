<script setup>
    ...
    const currentRoutePath = ref('/dashboard/doctor/appointments/upcoming')
    ...
    async function get_user_data(){
        try{
            const response = await axios_instance.get('/api/dashboard/doctor')
            localStorage.setItem("doctor_data", JSON.stringify(response.data))
            if(response.data.role !== 'doctor' || response.data.is_active === false){
                logout()
            }
            doc_name.value = response.data?.full_name
    ...
    async function searchQuery(){
        ...
        if(currentRoutePath.value === '/dashboard/doctor/appointments/upcoming'){
            ...
            globalTemp.set('searchQuery', searchString.value.trim())
            routeChangeCounter.value++
            router.push('/dashboard/doctor/assigned-patients')
        }
        else{
...
<div class="dashboard-wrapper">
    ...
    <aside class="sidebar py-4" :class="{ 'show': isSidebarOpen, 'collapsed': !isSidebarOpen }">
        ...
        <nav class="nav flex-column flex-grow-1 overflow-hidden">
            ...
            <router-link to="/dashboard/doctor/appointments" class="nav-link" :class="{ 'router-link-active': $route.path.includes('/dashboard/doctor/patient-treatment') }" title="Appointments"><i class="bi bi-calendar2-check me-3"></i><span class="sidebar-text">Appointments</span></router-link>

            <router-link to="/dashboard/doctor/assigned-patients" class="nav-link" :class="{ 'router-link-active': $route.path.includes('/dashboard/doctor/patient-history') }" title="Assigned Patients"><i class="bi bi-card-checklist me-3"></i><span class="sidebar-text">Assigned Patients</span></router-link>
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="d-flex align-items-center border-start ps-3">
                    <div class="text-end me-2 d-none d-md-block">
                        <div class="fw-bold small">{{ doc_name }}</div>
                    </div>
                    <img src="@/assets/favicon/icons8-doctor-100.png" class="rounded-circle" width="40" height="40" alt="Avatar">
                </div>
            </div>
        </header>
